<template>
  <div class="page-header">
    ...
    <button class="btn-secondary" :disabled="years.loading" @click="loadYears">Aktualisieren</button>
  </div>

  <div v-if="years.loading" class="state loading">Wird geladen …</div>
  <div v-else-if="years.error" class="state error">{{ years.error }}</div>

  <div v-else class="section">
    <div v-if="!tippYears.length" class="state empty">
      ...
      <table class="data">
        <thead>
          <tr>
            ...
            <th>Zeitraum</th>
            <th>Status</th>
            <th class="numeric">Reihenpreis</th>
            <th class="numeric">Mitglieder</th>
            <th class="numeric">Scheine</th>
            <th class="numeric">Ziehungen</th>
            <th class="numeric">Gewinne</th>
            <th></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="year in tippYears" :key="year.tippYearId">
            <td>#{{ year.tippYearId }}</td>
            <td>{{ year.name }}</td>
            <td>{{ formatDate(year.startDate) }} – {{ formatDate(year.endDate) }}</td>
            <td><span class="badge" :class="year.status">{{ statusLabel(year.status) }}</span></td>
            <td class="numeric">{{ formatAmount(year.ticketCostPerRow) }}</td>
            <td class="numeric">{{ year.memberCount }}</td>
            <td class="numeric">{{ year.ticketCount }}</td>
            <td class="numeric">{{ year.drawCount }}</td>
            <td class="numeric">{{ formatAmount(year.totalWinnings) }}</td>
            <td>
              <button class="btn-link" @click="select(year.tippYearId)">
    ...
    <form @submit.prevent="createTippYear">
      <div class="field-row">
        <div class="field">
          ...
          <input id="ty-name" v-model="newYear.name" required placeholder="Tippjahr 2026">
        </div>
        <div class="field">
          <label for="ty-start">Beginn</label>
          <input id="ty-start" v-model="newYear.startDate" type="date" required>
        </div>
        <div class="field">
          <label for="ty-end">Ende</label>
          <input id="ty-end" v-model="newYear.endDate" type="date" required>
        </div>
        <div class="field">
          <label for="ty-cost">Preis je Reihe und Ziehung</label>
          <input id="ty-cost" v-model="newYear.ticketCostPerRow" type="number" step="0.01" min="0" required>
        </div>
      </div>
      <button class="btn-primary" :disabled="createYear.pending" type="submit">
  ...
  <template v-if="selectedId">
    <h2 class="section-title">Tippjahr #{{ selectedId }}</h2>

    <!-- B-14 -->
    <div class="card section">
      <h3>Tippperioden</h3>

      <div v-if="periods.loading" class="state loading">Wird geladen …</div>
      <div v-else-if="periods.error" class="state error">{{ periods.error }}</div>
      <div v-else-if="!betPeriods.length" class="state empty">
        Noch keine Periode angelegt. Ohne Periode gibt es keine Tippreihe.
      </div>
      <div v-else class="table-wrap section">
        <table class="data">
          <thead>
            <tr>
              <th>ID</th>
              <th>Name</th>
              <th>Zeitraum</th>
              <th class="numeric">Folge</th>
              <th class="numeric">Reihen</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="period in betPeriods" :key="period.betPeriodId">
              <td>#{{ period.betPeriodId }}</td>
              <td>{{ period.name }}</td>
              <td>{{ formatDate(period.startDate) }} – {{ formatDate(period.endDate) }}</td>
              <td class="numeric">{{ period.sequence }}</td>
              <td class="numeric">{{ period.betRowCount }}</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <form @submit.prevent="createBetPeriod">
        <div class="field-row">
          <div class="field">
            ...
            <input id="bp-name" v-model="newPeriod.name" required placeholder="Q1 2026">
          </div>
          <div class="field">
            <label for="bp-start">Beginn</label>
            <input id="bp-start" v-model="newPeriod.startDate" type="date" required>
          </div>
          <div class="field">
            <label for="bp-end">Ende</label>
            <input id="bp-end" v-model="newPeriod.endDate" type="date" required>
          </div>
          <div class="field">
            <label for="bp-seq">Folge</label>
            <input id="bp-seq" v-model="newPeriod.sequence" type="number" min="1">
          </div>
        </div>
        <p class="state note">
        ...
        <button class="btn-primary" :disabled="createPeriod.pending" type="submit">
      ...
      <form @submit.prevent="addMember">
        <div class="field-row">
          <div class="field">
            ...
            <input id="m-participant" v-model="newMember.participantId" type="number" min="1" required>
          </div>
          <div class="field">
            <label for="m-joined">Beitritt</label>
            <input id="m-joined" v-model="newMember.joinedAt" type="datetime-local">
            <span class="hint">leer = jetzt</span>
          </div>
        </div>
        <button class="btn-primary" :disabled="addMemberCmd.pending" type="submit">
      ...
      <form @submit.prevent="submitTicket">
        <div class="field-row">
          <div class="field">
            ...
            <input id="t-start" v-model="newTicket.periodStart" type="date" required>
          </div>
          <div class="field">
            <label for="t-end">Zeitraum bis</label>
            <input id="t-end" v-model="newTicket.periodEnd" type="date" required>
          </div>
          <div class="field">
            <label for="t-draws">Ziehungen</label>
            <input id="t-draws" v-model="newTicket.drawCount" type="number" min="1" required>
          </div>
          <div class="field">
            <label for="t-sz">Superzahl</label>
            <input id="t-sz" v-model="newTicket.superzahl" type="number" min="0" max="9">
            <span class="hint">aus der Losnummer, gilt für alle Reihen</span>
          </div>
          <div class="field">
            <label for="t-ref">Losnummer / Referenz</label>
            <input id="t-ref" v-model="newTicket.lotteryReference">
          </div>
        </div>
        <p class="state note">
        ...
        <button class="btn-primary" :disabled="submitTicketCmd.pending" type="submit">
    ...
    <div class="card section">
      ...
      <form @submit.prevent="distributePayout">
        <div class="field">
          ...
          <input id="p-note" v-model="payout.note">
        </div>
        <label class="checkbox">
          <input v-model="payout.confirm" type="checkbox">
        ...
        <button class="btn-danger" :disabled="!payout.confirm || payoutCmd.pending" type="submit">
